...
const getSvelteVersion = require('./utils/get-svelte-version');
...
class ConfigGenerator {
    ...
    getWebpackConfig() {
        ...
        if (this.webpackConfig.useSvelte) {
            let svelteAlias = getSvelteVersion(this.webpackConfig) === 3 ? 'svelte' : 'svelte/src/runtime';
            config.resolve.alias['svelte'] = path.resolve('node_modules', svelteAlias);
